#
# This code is taken from Hadley Wickham's devtools package.
# Original: https://raw.github.com/hadley/devtools/master/R/rtools.r
#
# We need Rtools to be in the path to compile the ctsm models.
# Using this code will only require the users to install Rtools
# without worrying about their PATH variables.
#
# I will not simply depend on devtools as it in turn depends on 6
# other (for ctsmr unnecessary) CRAN packages.
#
...
scan_path_for_rtools <- function(debug = FALSE,
   ...
   if (gcc49) {
      ...
      cc_cmd <- paste(shQuote(file.path(R.home(component = "bin"), "R")),
                      "CMD config CC")
      
      cc_path <- withCallingHandlers(system(cc_cmd, intern = TRUE, ignore.stderr = !debug),
